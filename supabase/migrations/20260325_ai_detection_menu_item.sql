-- Add AI Detection menu item to all existing tenants
...
SELECT id, 'ai-detection', 'AI Detection', '/ai-detection', 'Radar', 11, true, 'main', NULL
FROM public.tenants
ON CONFLICT (tenant_id, menu_key) DO NOTHING;

-- Add Agent Hub menu item to all existing tenants
INSERT INTO public.menu_items (tenant_id, menu_key, original_label, route, icon, sort_order, is_visible, category, parent_menu_key)
SELECT id, 'agents', 'סוכני AI', '/agents', 'Cpu', 12, true, 'main', NULL
FROM public.tenants
ON CONFLICT (tenant_id, menu_key) DO NOTHING;

-- Update initialize function to include ai-detection for new tenants
...
BEGIN
  ...
  VALUES
    ...
    (_tenant_id, 'ai-detection', 'AI Detection', '/ai-detection', 'Radar', 11, true, 'main', NULL),
    (_tenant_id, 'agents', 'סוכני AI', '/agents', 'Cpu', 12, true, 'main', NULL),
