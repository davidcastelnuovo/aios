-- Social Gantt Posts table for social media content planning
...
-- RLS Policies: users can only access posts in their tenant
CREATE POLICY "Users can view social gantt posts in their tenant"
  ON public.social_gantt_posts
  FOR SELECT
  USING (tenant_id IN (
    SELECT tenant_id FROM public.tenant_users WHERE user_id = auth.uid()
  ));

CREATE POLICY "Users can create social gantt posts in their tenant"
  ON public.social_gantt_posts
  FOR INSERT
...
CREATE POLICY "Users can update social gantt posts in their tenant"
  ON public.social_gantt_posts
  FOR UPDATE
  USING (tenant_id IN (
    SELECT tenant_id FROM public.tenant_users WHERE user_id = auth.uid()
...
CREATE POLICY "Users can delete social gantt posts in their tenant"
  ON public.social_gantt_posts
  FOR DELETE
  USING (tenant_id IN (
    SELECT tenant_id FROM public.tenant_users WHERE user_id = auth.uid()
