-- Fix ambiguous overloaded RPC and ensure multi-salesperson filtering uses junction table

-- 1) Drop ALL overloaded versions to remove PostgREST ambiguity (PGRST203)
DROP FUNCTION IF EXISTS public.get_leads_by_stages(uuid, uuid[], text[], integer, integer, text, uuid, text[], uuid[], timestamptz, timestamptz);
DROP FUNCTION IF EXISTS public.get_leads_by_stages(uuid, uuid[], text[], integer, text, uuid, text[], boolean, timestamptz, timestamptz, uuid[]);
DROP FUNCTION IF EXISTS public.get_leads_by_stages(uuid, uuid[], text[], integer, integer, text, uuid[], text[], uuid[], timestamptz, timestamptz, boolean, timestamptz, timestamptz);
DROP FUNCTION IF EXISTS public.get_leads_by_stages(uuid, uuid[], text[], integer, integer, text, uuid, uuid[], text[], boolean, timestamptz, timestamptz, timestamptz, timestamptz, uuid[]);

-- 2) Recreate a single canonical function used by the frontend
...
BEGIN
  ...
  LOOP
    ...
    IF p_stages IS NULL OR stage_record.stage_key = ANY(p_stages) THEN
      ...
      -- Leads for this stage with limit and offset
