-- Update get_unknown_chat_contacts to also check campaigners table
  ...
  SELECT 
    ...
    COALESCE(
      (SELECT cm_match.campaigner_name 
       ...
       WHERE substring(regexp_replace(cm_match.campaigner_phone, '[^0-9]', '', 'g') from '.{9}$') = 
             substring(regexp_replace(pc.phone, '[^0-9]', '', 'g') from '.{9}$')
       LIMIT 1),
      inb.sender_name, 
      pc.phone
    ) as name,
