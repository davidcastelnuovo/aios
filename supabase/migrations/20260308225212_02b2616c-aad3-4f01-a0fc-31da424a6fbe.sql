CREATE OR REPLACE FUNCTION public.get_leads_by_stages(p_tenant_id uuid, p_agency_ids uuid[] DEFAULT NULL::uuid[], p_stages text[] DEFAULT NULL::text[], p_limit_per_stage integer DEFAULT 50, p_offset_per_stage integer DEFAULT 0, p_search_query text DEFAULT NULL::text, p_from_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_to_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_sales_person_ids uuid[] DEFAULT NULL::uuid[], p_response_statuses text[] DEFAULT NULL::text[], p_follow_up_today boolean DEFAULT false, p_start_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_end_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_tag_ids uuid[] DEFAULT NULL::uuid[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
