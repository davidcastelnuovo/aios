CREATE OR REPLACE FUNCTION public.get_chat_contacts(
  ...
  p_provider chat_provider DEFAULT NULL
)
...
BEGIN
  ...
  SELECT * FROM (
    -- Clients
    SELECT
      c.id AS contact_id, 'client'::text AS contact_type, c.name AS name, c.contact_name AS contact_name,
      c.phone AS phone, c.email AS email,
      c.agency_id AS agency_id, a.name AS agency_name,
      COALESCE((SELECT COUNT(*)::bigint FROM chat_messages cm
         ...
         AND (p_provider IS NULL OR cm.provider = p_provider)), 0) AS unread_count,
      (SELECT MAX(created_at) FROM chat_messages cm
         WHERE cm.client_id = c.id AND cm.connection_user_id = ANY(user_ids)
         AND (p_provider IS NULL OR cm.provider = p_provider)) AS last_message_at,
      false AS is_blocked, c.manychat_subscriber_id AS manychat_subscriber_id,
      c.active_chat_provider AS active_chat_provider, c.whatsapp_avatar_url AS whatsapp_avatar_url
    FROM clients c
    ...
    SELECT
      ...
      COALESCE((SELECT COUNT(*)::bigint FROM chat_messages cm
         ...
         AND cm.read_at IS NULL AND cm.is_blocked = false
         AND cm.connection_user_id = ANY(user_ids)
         AND (p_provider IS NULL OR cm.provider = p_provider)), 0),
      (SELECT MAX(created_at) FROM chat_messages cm
