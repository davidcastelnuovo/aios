-- Include campaign_name on kanban/chat payloads so every CRM view can show
-- the imported campaign in the table, cards, and lead-details "מקור" field.
-- Also search by campaign_name.
    ...
    IF p_stages IS NULL OR stage_record.stage_key = ANY(p_stages) THEN
      ...
      WHERE l.tenant_id = p_tenant_id
        ...
        AND (search_pattern IS NULL OR (
          ...
          lower(COALESCE(l.campaign_name, '')) LIKE search_pattern
        ))
