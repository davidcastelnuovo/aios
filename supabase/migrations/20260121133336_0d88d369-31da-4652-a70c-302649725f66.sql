-- Fix get_leads_by_stages to use correct table chat_contact_tags instead of non-existent lead_tags
DROP FUNCTION IF EXISTS public.get_leads_by_stages(UUID, UUID[], TEXT[], INT, TEXT, UUID, TEXT[], BOOLEAN, TIMESTAMPTZ, TIMESTAMPTZ, UUID[]);
...
BEGIN
  ...
  LOOP
    ...
    WITH filtered_lead_ids AS (
      SELECT DISTINCT cct.lead_id
      FROM chat_contact_tags cct
      WHERE p_tag_ids IS NOT NULL 
