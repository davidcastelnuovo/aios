-- Fix get_unknown_chat_contacts: use raw_provider_data instead of non-existent column
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(NULLIF(lm.sender_name, ''), lm.sender_phone) AS name,
    lm.sender_phone,
...
-- Fix search_contacts_for_chat: use g.whatsapp_avatar_url instead of g.avatar_url
DROP FUNCTION IF EXISTS public.search_contacts_for_chat(text, uuid);
