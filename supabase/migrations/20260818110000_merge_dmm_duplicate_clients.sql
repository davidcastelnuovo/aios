-- Merge three duplicate DMM client pairs (post CSV import cleanup).
-- Reuses the ABS merge pattern via merge_client_records().
...
BEGIN
  ...
  -- Agency may differ within tenant; survivor agency_id is kept.
  ...
  SET
    ...
    contact_name = coalesce(nullif(t.contact_name, ''), s.contact_name, t.contact_name),
    retainer = coalesce(t.retainer, s.retainer),
    monthly_budget = coalesce(t.monthly_budget, s.monthly_budget),
    tier = coalesce(t.tier, s.tier),
    mood_status = coalesce(t.mood_status, s.mood_status),
    services = (
      SELECT coalesce(jsonb_agg(DISTINCT elem ORDER BY elem), '[]'::jsonb)
      FROM (
        SELECT jsonb_array_elements_text(
          CASE
            WHEN t.services IS NULL OR t.services = 'null'::jsonb OR t.services = '[]'::jsonb
              OR (jsonb_typeof(t.services) = 'array' AND jsonb_array_length(t.services) = 0)
            THEN '[]'::jsonb ELSE t.services END
        ) AS elem
        UNION
        SELECT jsonb_array_elements_text(
          CASE
            WHEN s.services IS NULL OR s.services = 'null'::jsonb OR s.services = '[]'::jsonb
              OR (jsonb_typeof(s.services) = 'array' AND jsonb_array_length(s.services) = 0)
            THEN '[]'::jsonb ELSE s.services END
        ) AS elem
      ) u
...
SELECT public.merge_client_records(
  '7f4e478a-5c2c-4652-be96-72928c4b60fe',
  '313f11ef-a408-4e6c-82c0-31f73a216e1b',
  'עודד אביב duplicate: fold empty active shell into CSV-enriched record'
);

SELECT public.merge_client_records(
  '267d2f83-09c9-4f0d-973f-eb677254d929',
  'a93f8f40-dd24-49ff-8bb6-cc9788946647',
  'גלקסיה נכסים duplicate: fold paused MC shell into active LTD record with history'
);

SELECT public.merge_client_records(
  '76d2f6aa-9d35-437c-8611-b48b1c3c00fd',
  'd578f1db-f9be-44bf-9e4a-18b8921a0efb',
  'גיל פינקלשטיין duplicate: fold older record into fuller team/history survivor'
);
