...
-- Drop both versions and recreate a single canonical version
DROP FUNCTION IF EXISTS public.get_leads_by_stages(uuid, uuid[], text[], integer, integer, text, uuid[], text[], uuid[], timestamp with time zone, timestamp with time zone, boolean, timestamp with time zone, timestamp with time zone);
DROP FUNCTION IF EXISTS public.get_leads_by_stages(uuid, uuid[], text[], integer, text, timestamp with time zone, timestamp with time zone, uuid[], text[], boolean, timestamp with time zone, timestamp with time zone, uuid[], integer);
...
BEGIN
  ...
  LOOP
    ...
    IF p_stages IS NULL OR stage_record.stage_key = ANY(p_stages) THEN
      ...
      -- Leads for this stage with limit and offset - ORDER BY created_at DESC (newest first)
      SELECT COALESCE(jsonb_agg(lead_data ORDER BY (lead_data->>'created_at') DESC), '[]'::JSONB)
      INTO stage_leads
      FROM (
      ...
      result := result || jsonb_build_object(
        stage_record.stage_key, jsonb_build_object(
          'id', stage_record.id,
