-- Staging apply: Goal Execution Mode + Carmen Autonomous Goal Engine (all phases)
-- Recorded as: apply_goal_execution_and_autonomous_engine_staging
-- Staging-safe: skips dev_tasks (table may not exist on Staging clone).

-- === Goal Execution Mode (base) ===
...
-- === Carmen Autonomous Goal Engine (phases 1–4) ===
ALTER TABLE public.goals
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'goals_engine_status_check') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'goals_risk_level_check') THEN
    ALTER TABLE public.goals ADD CONSTRAINT goals_risk_level_check
...
DROP POLICY IF EXISTS goal_success_criteria_tenant_rw ON public.goal_success_criteria;
...
DROP POLICY IF EXISTS goal_loop_iterations_tenant_read ON public.goal_loop_iterations;
CREATE POLICY goal_loop_iterations_tenant_read ON public.goal_loop_iterations
  FOR SELECT USING (
    tenant_id IN (SELECT tu.tenant_id FROM public.tenant_users tu WHERE tu.user_id = auth.uid())
  );

DROP POLICY IF EXISTS goal_plan_steps_tenant_rw ON public.goal_plan_steps;
...
DROP POLICY IF EXISTS goal_actions_tenant_read ON public.goal_actions;
CREATE POLICY goal_actions_tenant_read ON public.goal_actions
  FOR SELECT USING (
    tenant_id IN (SELECT tu.tenant_id FROM public.tenant_users tu WHERE tu.user_id = auth.uid())
  );

DROP POLICY IF EXISTS goal_evidence_tenant_read ON public.goal_evidence;
CREATE POLICY goal_evidence_tenant_read ON public.goal_evidence
  FOR SELECT USING (
    tenant_id IN (SELECT tu.tenant_id FROM public.tenant_users tu WHERE tu.user_id = auth.uid())
  );

DROP POLICY IF EXISTS goal_model_events_tenant_read ON public.goal_model_events;
CREATE POLICY goal_model_events_tenant_read ON public.goal_model_events
  FOR SELECT USING (
    tenant_id IN (SELECT tu.tenant_id FROM public.tenant_users tu WHERE tu.user_id = auth.uid())
  );

DROP POLICY IF EXISTS goal_engine_tools_tenant_rw ON public.goal_engine_tools;
...
-- Worker cron (Staging project URL substituted at apply time)
DO $$
DECLARE
  existing_job_id bigint;
  worker_url text := '__STAGING_SUPABASE_URL__/functions/v1/autonomous-goal-worker';
BEGIN
