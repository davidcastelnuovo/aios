...
### ⚠️ קיבלת 3 הודעות עם אותם פרטים?

| סיבה | מה לבדוק |
|---|---|
| Make שולח את ה-Webhook **3 פעמים** (Retry / מסלולים מקבילים) | AIOS מדלג עכשיו על אותו ליד תוך 90 שניות — בדוק ב-Make שיש **מודול HTTP אחד** |
| **Rule נפרד** על טאג `aios_lead_alert` **בנוסף** ל-Flow | Automations → מחק/השבת Rule כפול; השאר רק Flow «ליד חדש ללקוח» (sendFlow מ-AIOS) |
| Flow עם **יותר מצומת Send Template אחד** | ערוך Flow — תבנית אחת בסוף בלבד |
| 3 בדיקות שונות אבל **אותם שדות** ב-Make | שנה `lead_name` / `lead_phone` בכל טסט |
