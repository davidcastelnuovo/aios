...
- Structural differences fail preflight instead of replaying Production's migration history over Staging. Compatibility SQL adds the observed missing report columns and extends the exact legacy asset-type constraint to admit storyboard assets already produced by the creative code. It preserves newer/custom constraints.
...
- Legacy IDs are aligned only for reviewed, unambiguous logical keys in module permissions, menu items, monthly SEO updates and global skills. The transaction refuses unreviewed inbound foreign keys or a canonical ID already in use, preserves record fields, records the ID mapping privately and suppresses user-trigger side effects. Route/tag IDs are aligned together with their explicitly reviewed agent/conversation/contact-tag references. Foreign-key checks are deferred inside that transaction, then validated and restored to their original definitions before commit. Any unexpected reference aborts. Conflicting Carmen instructions remain explicit rejections.
