...
- **Guaranteed update channel:** when a Claude routine session finishes a Carmen-delegated task, it pushes a WhatsApp update straight to David by running `select public.claude_notify_david('<message>');` (or with an explicit tenant) via its Supabase connector. That SECURITY DEFINER function reads the shared bearer from Vault and calls the `claude-notify` edge function, which sends through the tenant's Carmen WhatsApp automation — independent of any live Carmen session.
- **Default notify / working org = Marketing Captain.** Always use MarketingCaptain (`2dcdaac6-41bf-42cc-86bf-9a0b4b2e6019`) and Carmen of Marketing Captain for WhatsApp updates and default work context. `claude_notify_david`'s DB default is already this UUID — **do not override it** with a client’s tenant (e.g. ABS lives under DMM). Only use DMM (or another org) when David says so explicitly.
