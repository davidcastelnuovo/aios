...
import { User, Phone, Building2, Clock, Search, Mail, Globe, CheckSquare, Trash2, MessageSquare, FileText, DollarSign, X, Edit } from "lucide-react";
...
export function ClientsChatView({
  ...
  const handleBulkMoodChange = async (moodStatus: string) => {
    ...
    setBulkActionLoading(true);
    try {
      const { error } = await supabase.from("clients").update({ mood_status: moodStatus }).in("id", Array.from(selectedClientIds));
      if (error) throw error;
      toast.success(`${selectedClientIds.size} לקוחות עודכנו`);
      exitMultiSelect();
      queryClient.invalidateQueries({ queryKey: ["clients"] });
        ...
        {selectedClient ? (
          <>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
              <TabsList className="mx-4 mt-3 grid grid-cols-3 w-auto max-w-lg h-9 bg-muted/50 mr-4 ml-auto">
